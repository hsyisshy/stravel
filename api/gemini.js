 ...
 * 1. AI 智慧排程生成器
 */
async function generateItinerary(ai, { destination, days, departureDate, style, notes }) {
...
2. 日期從 ${departureDate || '2026-09-01'} 起算連續 ${days || 1} 天。`
  ...
  return parseJsonResponse(response.text, 'AI 生成行程格式解析失敗，請重試一次。')
}

/**
 * 2. AI 突發狀況動態調程與應變公告草擬
 */
async function replanItinerary(ai, { currentItinerary, situation, targetDate, groupNotes }) {
...
請回傳一個嚴格的 JSON 物件，格式如下：
{
  ...
  return parseJsonResponse(response.text, 'AI 調程解析失敗，請重試一次。')
}

/**
 * 3. AI 公告撰寫與潤飾助手
 */
async function draftAnnouncement(ai, { rawTopic, category, groupInfo }) {
...
請回傳嚴格 JSON：
{
  ...
  return parseJsonResponse(response.text, 'AI 公告草擬失敗，請重試。')
}

/**
 * 4. 多模態拍照識景 + AI 語音導覽解說 (Multimodal Vision)
 */
async function narrateLandmark(ai, { imageBase64, mimeType = 'image/jpeg', locationName, userQuestion, groupInfo }) {
...
請以繁體中文（台灣）回傳嚴格 JSON 物件：
{
  ...
  return parseJsonResponse(response.text, 'AI 景點辨識與導覽解析失敗，請重試一次。')
}

/**
 * 5. 專屬團務 RAG AI 智能小幫手 (Chatbot)
 */
async function chatTourAssistant(ai, { message, history = [], groupContext }) {
 ...
 */
async function generateTravelStory(ai, { photos = [], itinerary = [], groupInfo, tone = '感人溫馨' }) {
